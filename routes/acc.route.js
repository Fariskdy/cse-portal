...
router.get('/expenses-data', async (req, res) => {
  ...
  res.render('dashboard/acc/years', { years });
});
  ...
  Expense.find({ month: { $regex: year } })
    ...
    .exec((err, months) => {
      if (err) {
        ...
        res.render('dashboard/acc/months', { year, months });
      }
    });
});
      ...
      Expense.find({ month })
        ...
        .exec((err, regions) => {
          if (err) {
            ...
            console.log(year);
            console.log(month);
            res.render('dashboard/acc/regions', { month, regions, year, unverifiedCounts });
          }
        });
    }
  });
  ...
  Expense.find({ month, region })
    .exec((err, expenses) => {
      if (err) {
        ...
        res.render('dashboard/acc/data', { year,month, region, expenses });
      }
    });
});
...
router.get('/settings', (req, res, next) => {
  res.render('dashboard/acc/settings');
}
);
...
router.post('/change-password', async (req, res) => {
  ...
  try {
    ...
    if (!isPasswordValid) {
      ...
      return res.redirect('/acc/settings');
    }
...
router.get('/region-data', async (req, res) => {
  try {
    ...
    res.render('dashboard/acc/region-data', { regions });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
...
router.get('/region-data/:region', async (req, res) => {
  try {
    ...
    res.render('dashboard/acc/region-data-details', { regionData, users });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
...
router.get('/region-data/:region/summary', async (req, res) => {
  try {
    ...
    if (regionData) {
      ...
      res.render('dashboard/acc/summary', {currentUserRegion,sdfAllottedAmount, examAllottedAmount , rangeAllottedAmount , artsAllottedAmount ,
      ...
      res.render('dashboard/acc/summary', { sdfAllottedAmount: 0 });
    }
  } catch (error) {
    console.error(error);
    res.redirect('/error');
...
router.get('/criteria', (req, res, next) => {
  res.render('dashboard/acc/criteria');
})
