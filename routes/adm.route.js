...
router.get('/expenses', async (req, res) => {
  ...
  res.render('dashboard/adm/expenses', { expensesByRegion });
});
...
router.get('/expenses/:region', async (req, res) => {
  ...
  const expenses = await Expense.find({
    ...
    expenses.forEach(expense => {
      if (expense.verifiedByFcm && expense.verifiedByDop && expense.verifiedByAdm) {
        totalAmount += expense.amount;
      }
    });
    
  
    res.render('dashboard/adm/expense-detail', { region: req.params.region, expenses, totalAmount });
  });
  
// Administrator Verification(sanction)  
router.post('/expenses/:id/verify', async (req, res) => {
    try {
      const expense = await Expense.findById(req.params.id);
      expense.verifiedByAdm = true;
      await expense.save();
      req.flash('success', 'Expense verified successfully.');
      res.redirect(`/adm/expenses/${expense.region}`); // Redirect to the expenses page
    } catch (err) {
      console.error(err);
...
router.get('/pre-expenses', async (req, res) => {
  ...
  res.render('dashboard/adm/pre-expenses', { expensesByRegion });
});
...
router.get('/pre-expenses/:region', async (req, res) => {
  ...
  res.render('dashboard/adm/pre-expense-detail', { region: req.params.region, expenses, totalAmount });
});
...
router.post('/pre-expenses/:id/verify', async (req, res) => {
  try {
    ...
    await expense.save();
    req.flash('success', 'Expense verified successfully.');
    res.redirect(`/adm/pre-expenses/${expense.region}`); // Redirect to the expenses page
  } catch (err) {
    console.error(err);
    req.flash('error', 'Failed to verify the expense.');
    res.status(500).send('Server error');
...
router.get('/expenses-data', async (req, res) => {
  ...
  res.render('dashboard/adm/years', { years });
});
  ...
  Expense.find({ month: { $regex: year } })
    ...
    .exec((err, months) => {
      if (err) {
        ...
        res.render('dashboard/adm/months', { year, months });
      }
    });
});
      ...
      Expense.find({ month })
        ...
        .exec((err, regions) => {
          if (err) {
            ...
            console.log(year);
            console.log(month);
            res.render('dashboard/adm/regions', { month, regions, year, unverifiedCounts });
          }
        });
    }
  });
  ...
  Expense.find({ month, region })
    .exec((err, expenses) => {
      if (err) {
        ...
        res.render('dashboard/adm/data', { year,month, region, expenses });
      }
    });
});
...
router.get('/settings', (req, res, next) => {
  res.render('dashboard/adm/settings');
}
);
// update password
...
router.get('/region-data', async (req, res) => {
  try {
    ...
    res.render('dashboard/adm/region-data', { regions });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
...
router.get('/region-data/:region', async (req, res) => {
  try {
    ...
    res.render('dashboard/adm/region-data-details', { regionData, users });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
...
router.get('/region-data/:region/summary', async (req, res) => {
  try {
    ...
    if (regionData) {
      ...
      res.render('dashboard/adm/summary', {currentUserRegion,sdfAllottedAmount, examAllottedAmount , rangeAllottedAmount , artsAllottedAmount ,
      ...
      res.render('dashboard/adm/summary', { sdfAllottedAmount: 0 });
    }
  } catch (error) {
    console.error(error);
    res.redirect('/error');
...
router.get('/criteria', (req, res, next) => {
  res.render('dashboard/adm/criteria');
})
