...
router.get('/expenses', async (req, res) => {
  ...
  res.render('dashboard/dop/expenses', { expensesByRegion });
});
...
router.get('/expenses/:region', async (req, res) => {
  ...
  const expenses = await Expense.find({
    ...
    expenses.forEach(expense => {
      if (expense.verifiedByFcm && expense.verifiedByDop) {
        totalAmount += expense.amount;
      }
    });
    
  
    res.render('dashboard/dop/expense-detail', { region: req.params.region, expenses, totalAmount });
  });
// Dop verification of expenses
router.post('/expenses/:id/verify', async (req, res) => {
    try {
      const expense = await Expense.findById(req.params.id);
      expense.verifiedByDop = true;
      await expense.save();
      req.flash('success', 'Expense verified successfully.');
      res.redirect(`/dop/expenses/${expense.region}`); // Redirect to the expenses page
    } catch (err) {
      console.error(err);
...
router.get('/pre-expenses', async (req, res) => {
  ...
  res.render('dashboard/dop/pre-expenses', { expensesByRegion });
});
...
router.get('/pre-expenses/:region', async (req, res) => {
  ...
  res.render('dashboard/dop/pre-expense-detail', { region: req.params.region, expenses, totalAmount });
});
// DOP verification for the expenses
router.post('/pre-expenses/:id/verify', async (req, res) => {
  try {
    ...
    await expense.save();
    req.flash('success', 'Expense verified successfully.');
    res.redirect(`/dop/pre-expenses/${expense.region}`); // Redirect to the expenses page
  } catch (err) {
    console.error(err);
    req.flash('error', 'Failed to verify the expense.');
    res.status(500).send('Server error');
...
router.get('/expenses-data', async (req, res) => {
  ...
  res.render('dashboard/dop/years', { years });
});
  ...
  Expense.find({ month: { $regex: year } })
    ...
    .exec((err, months) => {
      if (err) {
        ...
        res.render('dashboard/dop/months', { year, months });
      }
    });
});
      ...
      Expense.find({ month })
        ...
        .exec((err, regions) => {
          if (err) {
            ...
            console.log(year);
            console.log(month);
            res.render('dashboard/dop/regions', { month, regions, year, unverifiedCounts });
          }
        });
    }
  });
  ...
  Expense.find({ month, region })
    .exec((err, expenses) => {
      if (err) {
        ...
        res.render('dashboard/dop/data', { year,month, region, expenses });
      }
    });
});
...
router.get('/settings', (req, res, next) => {
  res.render('dashboard/dop/settings');
}
);
// update password
...
router.post('/change-password', async (req, res) => {
  ...
  try {
    ...
    if (!isPasswordValid) {
      ...
      return res.redirect('/dop/settings');
    }
...
router.get('/region-data', async (req, res) => {
  try {
    ...
    res.render('dashboard/dop/region-data', { regions });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
...
router.get('/region-data/:region', async (req, res) => {
  try {
    ...
    res.render('dashboard/dop/region-data-details', { regionData, users });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
  }
...
router.get('/region-data/:region/summary', async (req, res) => {
  try {
    ...
    if (regionData) {
      ...
      res.render('dashboard/dop/summary', {currentUserRegion,sdfAllottedAmount, examAllottedAmount , rangeAllottedAmount , artsAllottedAmount ,
      ...
      res.render('dashboard/dop/summary', { sdfAllottedAmount: 0 });
    }
  } catch (error) {
    console.error(error);
    res.redirect('/error');
...
router.get('/criteria', (req, res, next) => {
  res.render('dashboard/dop/criteria');
})
